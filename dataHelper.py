...
#        if self.conf.dataset.startswith("movies"):
#            stamp2date = lambda stamp :datetime.datetime.fromtimestamp(stamp)
#            df["date"]= df["timestamp"].apply(stamp2date).apply(stamp2date).dt.strftime(date_format="%Y-%m-%d") 
#            
#        else:
#            df["date"]= df["timestamp"]
        ...
        if not self.conf.rating_flag :
            df["rating"]=(df["rating"]>4.99).astype('int')
            df=df[df.rating >  0.5]
