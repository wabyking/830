...
with g1.as_default():
    ...
    sess1.run(init1)
#    checkpoint_filepath= "model/Gen_gan/joint-25-0.29333-0.29600.ckpt"
...
def main(checkpoint_dir="model/"):
    ...
    #                myfile.write("pg loss : %.5f reward : %.5f "%(np.mean(np.array(pg_losses)),np.sum(np.array(rewardes)))+"\n")                                   
                ...
                if curentt_p5_score > best_p5:        	
                    ...
                    print("best p5 score %.5f"% best_p5)
                    checkpoint_dir=    "model/Gen_gan/"
                    helper.create_dirs(checkpoint_dir)                
                    saver1.save(sess1, checkpoint_dir + '%s-%d-%.5f-%.5f.ckpt'% (FLAGS.model_type,FLAGS.re_rank_list_length,scores[0][1], scores[1][1]))
